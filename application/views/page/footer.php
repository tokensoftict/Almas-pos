...
<div id="form-bp1" tabindex="-1" role="dialog" class="modal fade colored-header colored-header-primary">
	...
	function checkIFProductCode(code){
	    var dept = $('#department__').val();
	    if(dept ==""){
            $.gritter.add({
                title: 'Error',
                text: "Please select department to check barcode status",
                class_name: 'color danger'
            });
        }else {
            $.post(<?php echo "'" . base_url('dashboard/checkifBarcodeExist/') . "'+code+'/'+dept"  ?>, function (data) {
...
<script type="text/javascript">					
	 ...
	 <?php
if($this->users->get_user_by_username($this->session->userdata("username"))->role!="Sales Representative"  && $this->users->get_user_by_username($this->session->userdata("username"))->role!="Service Administrator"){
 ?>	
	...
	<?php
	}elseif($page=="new_stock" || $page=="edit_stock" || $page=="newrma" || $page=="newexpenses" || $page=="edit_customer"  || $page=="editexpenses" || $page=="new_bank_deposit_withdraw" || $page=="new_assets" || $page=="edit_assets"){
	...
	<?php
	}elseif($page=="credit_report/credit_reports" || $page=="view_received" || $page=="view_invoice" || $page=="credit_report/credit_report_payment_method" || $page=="credit_history_by_customer" || $page=="credit_report/credit_report_sales_rep" || $page=="credit_report/credit_report_customer" || $page=="credit_report/credit_reports" || $page=="invoice_report/invoice_report_payment_method" || $page=="invoice_report/invoice_report_sales_rep" || $page=="invoice_report/invoice_report_customer" || $page=="invoice_report/invoice_reports" || $page=="stock" || $page=="stock_branch" || $page=="stockby_department" || $page=="list_service" || $page=="settings" || $page=="view_stock_list" || $page=="stock_transfer" || $page=="customerlist" || $page=="customercreditlog" || $page=="credits" || $page=="sales" || $page=="invoices" || $page=="view_transfer" || $page=="rma" || $page=="stock_recieved" || $page=="recieved_stock_report"|| $page=="stock_pick_up_report" || $page=="transfer_stock_report" || $page=="sales_report" || $page=="sales_reports" || $page=="report_customer"|| $page=="report_sales_rep" || $page=="report_stock" || $page=="invoice_report" || $page=="report_payment_method" || $page=="deposit_reports" || $page=="deposit_report_customer" || $page=="deposit_report_sales_rep" || $page=="deposit_report_payment_method" || $page=="customersalesreport_break_down"
	|| $page=="view_stock_movement" || $page=="expenses"   || $page=="staff_salary"  || $page=="new_salary_payment"  || $page=="view_history_salary" || $page=="profit_loss_general" || $page=="total_income_report" || $page=="by_report_customer" || $page=="by_report_sales_rep" || $page=="by_report_stock" || $page=="by_report_payment_method" || $page=="stock_expiry"  || $page=="expenses_monthly_report" || $page=="report_payment_method" || $page=="refund_report" || $page=="deposit_credit_report"  || $page=="assets_list"   || $page=="credit_payment_report" || $page=="customer_credit_payment_history"   || $page=="admin_refund_report" || $page=="stock_open_close"  || $page=="supplier_invoice_report"   || $page=="due_payment_report"   || $page=="out_of_stock"  || $page=="cash_ledger" || $page=="pos_payment"  || $page=="payment_method_sales_report" || $page=="vat_report"  || $page=="recieved_stock_report_byproduct" || $page=="sales_product" || $page=="daily_stock_balance" || $page=="customersalesreport"
	){
	?>
	...
	?>
        ...
        <script type="text/javascript">
            ...
            function addTemp(btn){
                var child = $("#produt_list tr");
                var num = child.length + 1;
                $(btn).attr('disabled','disabled');
                $(btn).html('Please wait');
                ajaxSentRequest('<?php
                    if($page == "new_transfer") {
                        echo base_url('dashboard/get_available_stock/');
                    }else{
                        echo base_url('dashboard/get_recieved_stock/');
                    }
                    ?>'+num,{},function(select){
                    ...
                    html +='<td><div class="input-group"><input id="product_qty_'+num+'" value="0" required type="number" name="qty[]" value="" placeholder="Quantity" class="form-control input-sm qty"></td>';
	...
	<?php
	}elseif($page=="edit_recieved" || $page=="new_recieved_branch" || $page=="new_payment_invoice" || $page=="new_invoice" || $page=="new_recieved_supplier" || $page=="edit_transfer"  || $page=="refund_depsits" || $page=="add_deposits" || $page=="add_new_deposit_history" || $page=="new_payment_credit" || $page=="clear_credit_payment"){
	?>
	 <script src="<?php  echo base_url() ?>assets/lib/fuelux/js/wizard.js" type="text/javascript"></script>
	<script src="<?php echo base_url('assets/lib/select2/js/select2.min.js') ?>" type="text/javascript"></script>
	<script src="<?php echo base_url('assets/lib/moment.js/min/moment.min.js'); ?>" type="text/javascript"></script>
	...
	callback(data,callbackdata)
	 ...
	 function addTemp(btn){		
		...
		var html='';
			html +='<tr>'
				html +='<td>'+select+'<input type="hidden" name="bar_code[]" id="bar_code-'+num+'"/></div></td>';
				html +='<td><div class="input-group"><input id="product_qty_'+num+'" value="0" required type="number" name="qty[]" value="" placeholder="Quantity" class="form-control input-sm"></td>';
				<?php
				if($page=="edit_transfer"){
				?>
