...
function checktaxable($cat_id){
	$this->db->from("category");
	$this->db->where("SN",$cat_id);
	$cat = $this->db->get()->row();
	if(isset($cat->tax)){
	if($cat->tax == "1"){
		return true;
...
function calculate_tax($products,$vat){
	$tax_amt = 0;
	foreach($products as $product){
		$stock = $this->stock->getStockByID($product['id']);
		if(is_array($stock)){
			if($this->checktaxable($stock['category_id']) == true){
				$cost_price = $product['total_cost_price'];
				$tax = ($vat/100) * $cost_price;			
				$tax_amt = $tax_amt + $tax;
			}
		}
	}
