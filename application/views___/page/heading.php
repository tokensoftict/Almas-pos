  ...
  <div class="be-wrapper be-fixed-sidebar">
	<nav class="navbar navbar-default navbar-fixed-top be-top-header">
        <div class="container-fluid">
		  ...
		  <a href="<?php echo base_url(base_url('admin/dashboard'))  ?>" class="navbar-brand">
			<img src="<?php echo $this->settings->getSettings()['slogo'];  ?>" width="120" />
		  </a></div>
		  ...
		  ?></b></span>
			...
			echo ($this->stock->getUserDepartmentPos()=="" ? $this->stock->getUserDepartment() : $this->stock->getUserDepartmentPos());
		  ?></b></span>
...
<?php  
	...
	if($user['role'] == "Administrator"){
		...
		$this->load->view("page/managersidelinks");
	}else if($user['role'] == "Accountant"){
		$this->load->view("page/accountsidelinks");
	}else if($user['role'] == "Sales Representative"){
		$this->load->view("page/salessidelinks");
	}else if($user['role'] == "Auditor"){
		$this->load->view("page/auditorsidelinks");
	}else if($user['role'] == "Stock Officer"){
		$this->load->view("page/stockofficersidelinks");
	}else if($user['role'] == "Service Administrator"){
        $this->load->view("page/serviceadminsidelinks");
    }
